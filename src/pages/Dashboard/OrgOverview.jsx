...
export default function OrgOverview({
  ...
  return (
    <Card variant="default" padding="p-5">
      ...
      {/* Organization Name */}
      <div className="mb-4 p-3 rounded-lg bg-gradient-to-br from-primary/5 to-violet-50 dark:from-blue-500/5 dark:to-violet-500/10 dark:border-primary/20 border border-primary/10">
        ...
        {stats.map((stat, i) => {
          ...
          return (
            <motion.div
              ...
              className="p-3 rounded-lg bg-slate-50 dark:dark-card-metallic border border-slate-100 dark:border-slate-600/20"
            >
              <div className="flex items-center gap-2 mb-1">
                <div className={`p-1 rounded-md ${colorMap[stat.color]} dark:bg-slate-800/80`}>
                  <Icon size={12} />
                </div>
                <span className="text-[10px] font-medium text-slate-500 dark:text-slate-400 uppercase">{stat.label}</span>
              </div>
              <p className="text-lg font-bold text-foreground dark:text-slate-100">{stat.value}</p>
            </motion.div>
          );
        })}
      </div>
        ...
        <div>
          ...
          <div className="space-y-1.5">
            {recentMembers.slice(0, 4).map((m, i) => (
              <div key={m.id || i} className="flex items-center gap-2.5 p-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[10px] text-slate-400 dark:text-slate-500">{m.role || 'staff'}</p>
                </div>
