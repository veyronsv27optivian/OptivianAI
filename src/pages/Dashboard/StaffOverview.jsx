...
export default function StaffOverview({
  ...
  return (
    <Card variant="default" padding="p-5">
      ...
      {/* Online Status */}
      <div className="flex items-center gap-3 mb-4 p-3 rounded-lg bg-gradient-to-br from-emerald-50 to-teal-50 border border-emerald-100">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500">out of {staffCount} total staff</p>
        </div>
      </div>
        ...
        {statusData.map((s, i) => {
          ...
          return (
            <div key={s.label} className="flex items-center gap-3">
              ...
              <span className="text-xs font-semibold text-slate-700 w-8 text-right">{s.value}</span>
            </div>
          );
        })}
      </div>
        ...
        {roleDistribution.map((role, i) => (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-600">{role.name}</span>
            </div>
